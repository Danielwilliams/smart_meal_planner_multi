...
function CartPage() {
  ...
  const handleAddToCart = async (items, store) => {
    ...
    console.error("⚠️ DEBUG: Critical error in CartPage:", err);
    return (
      <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
        <Typography variant="h4" color="error" gutterBottom>Error Loading Shopping Cart</Typography>
        <Card sx={{ p: 3, mb: 4, bgcolor: '#ffebee' }}>
          <Typography variant="h6">An error occurred while rendering the Cart page</Typography>
          <Typography component="pre" sx={{ mt: 2, p: 2, bgcolor: '#f5f5f5', borderRadius: 1, whiteSpace: 'pre-wrap' }}>
            {err.toString()}
            {err.stack}
          </Typography>
          <Button 
