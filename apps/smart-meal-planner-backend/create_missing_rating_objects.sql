-- Create missing database objects for rating system

-- 1. Create the get_or_create_recipe_interaction function
...
-- 2. Create recipe_ratings_summary view
...
-- 3. Create user_rating_preferences view
CREATE OR REPLACE VIEW user_rating_preferences AS
SELECT 
    user_id,
    COUNT(*) as total_ratings,
    AVG(rating_score) as avg_rating,
...
-- GRANT SELECT ON recipe_ratings_summary TO your_app_user;
-- GRANT SELECT ON user_rating_preferences TO your_app_user;
-- GRANT EXECUTE ON FUNCTION get_or_create_recipe_interaction TO your_app_user;

-- Verify the objects were created
SELECT 'Function created' as status, 'get_or_create_recipe_interaction' as object_name
WHERE EXISTS (
...
UNION ALL
SELECT 'View created' as status, 'recipe_ratings_summary' as object_name
WHERE EXISTS (
    SELECT 1 FROM pg_views WHERE schemaname = 'public' AND viewname = 'recipe_ratings_summary'
)
UNION ALL
SELECT 'View created' as status, 'user_rating_preferences' as object_name
WHERE EXISTS (
