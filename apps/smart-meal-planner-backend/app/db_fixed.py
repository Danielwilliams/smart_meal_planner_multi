...
# For backward compatibility, include all the functions from the original db.py
...
def get_user_saved_recipes(user_id):
    ...
    try:
        ...
        with get_db_cursor(pool_type='read', autocommit=True) as (cur, conn):
            cur.execute("""
...
def is_recipe_saved(user_id, menu_id, recipe_id=None, meal_time=None):
    ...
    try:
        ...
        with get_db_cursor(dict_cursor=False, pool_type='read', autocommit=True) as (cur, conn):
