...
def get_user_organization_id(user_id: int) -> int:
    ...
    try:
        with get_db_cursor(dict_cursor=False, autocommit=True) as (cur, conn):
            ...
            result = cur.fetchone()
            if not result:
                logger.error(f"User {user_id} attempted to access organization settings but is not an owner")
                raise HTTPException(
        ...
        raise HTTPException(
            ...
            detail="Error determining organization access"
        )
...
async def get_organization_settings(
    ...
    try:
        with get_db_cursor(dict_cursor=False) as (cur, conn):
            ...
            # Convert None to empty dict for JSON fields
            default_prefs = result[2] or {}
            operating_hours = result[8] or {}

            return OrganizationSettings(
                id=result[0],
                organization_id=result[1],
                default_client_preferences=default_prefs,
...
async def get_default_client_preferences(
    ...
    try:
        with get_db_cursor(dict_cursor=False, autocommit=True) as (cur, conn):
            ...
            if not cur.fetchone():
                logger.warning(f"User {current_user['user_id']} attempted to access organization {organization_id} preferences without permission")
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Access denied to this organization"
                )
            ...
            # Ensure we handle None values correctly
            default_prefs = result[0] if result[0] is not None else {}
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch default preferences: {str(e)}"
        )